 ...
 * 《易命之书》52条人生法则 — 知识库实现
 * 
 * 从原 yiming.ts 提取，适配 KnowledgeBase 接口
 */
...
const RULES: Rule[] = [
  ...
  },
  {
    ...
    category: "修身",
    keywords: ["作息", "规律", "早睡早起", "时间", "节奏", "饮食", "生物钟", "身体", "睡眠", "失眠", "疲劳", "精力", "养生", "调理", "虚", "弱", "疲惫"]
  },
  {
    id: 3, title: "言讷而实",
  ...
  },
  {
    ...
    text: "怒则智散，亦损气数。",
    category: "修身",
    keywords: ["愤怒", "脾气", "情绪控制", "发火", "暴躁", "冷静", "气", "气血", "心神", "内耗", "伤身", "情绪病", "压力", "焦虑", "紧张"]
  },
  {
    id: 38, title: "坠壑图出",
  ...
  },
  {
    ...
    category: "修身",
    keywords: ["反思", "自省", "内疚", "自责", "改正", "检讨", "自身原因", "心理负担", "精神压力", "愧疚", "悔恨", "释怀"]
  },
  {
    id: 52, title: "察起心动念",
    text: "观喜怒于己身，察起心动念，此乃运中之至运也。",
    category: "修身",
    keywords: ["觉察", "念头", "情绪", "内心", "自我认知", "正念", "心", "意念", "心理", "精神", "心态调整", "冥想", "静心", "心身"]
  },
  ...
  },
  {
    ...
    text: "勿自怜，时运亦厌之。",
    category: "心态",
    keywords: ["自怜", "自怨自艾", "消极", "悲观", "丧", "内耗", "抑郁", "低落", "沮丧", "自我否定", "心力交瘁", "情绪低落"]
  },
  {
    id: 40, title: "灵根身树运叶",
  ...
  },
  {
    ...
    category: "运势",
    keywords: ["运势", "乘势", "谦虚", "低调", "好运", "把握", "气运", "气场", "身体状态", "精气神", "能量", "活力"]
  },
  {
    id: 18, title: "谋宜见远忘私",
...
// 为所有规则添加知识库标识
RULES.forEach(r => { r.knowledgeBase = 'yiming'; });

// ========== 语义映射表（常见问题的快捷匹配） ==========
const SEMANTIC_MAP: Record<string, (number | string)[]> = {
  "压力": [2, 34, 37, 49, 52],
  "失眠": [2, 34, 49, 52],
  "焦虑": [5, 28, 31, 37, 49],
  "迷茫": [38, 40, 44, 46, 51],
 ...
 * 增强的搜索算法（含语义映射）
 */
function enhancedSearch(query: string, topK: number): Rule[] {
  ...
  const scored = RULES.map((rule) => {
    ...
    for (const kw of rule.keywords) {
      if (kw.includes(lowerQuery) || lowerQuery.includes(kw)) {
        ...
        for (const qg of queryBigrams) {
          if (kw.includes(qg) || qg.includes(kw)) { score += 2; break; }
        }
      }
    }

    // 类别关键词匹配
    const catKws = CATEGORY_MAP[rule.category] || [];
    for (const ck of catKws) {
      if (ck.includes(lowerQuery) || lowerQuery.includes(ck)) {
        score += 3;
      } else {
        for (const qg of queryBigrams) {
          if (ck.includes(qg) || qg.includes(ck)) { score += 1; break; }
        }
      }
    }

    // 语义映射
    for (const [senseKey, ruleIds] of Object.entries(SEMANTIC_MAP)) {
...
// ========== 导出知识库实例 ==========
export const yimingKnowledgeBase: KnowledgeBase = {
  id: 'yiming',
  name: '易命之书',
  description: '《易命之书》52条人生法则',
  getAllRules: () => [...RULES],
  searchRules: (query: string, topK: number) => enhancedSearch(query, topK),
